...
def menu_integrales_definidas():
    ...
    if opcion == '1':
        ...
        print(f"✅ Volumen (teórico) entre {a} y {b} es: {volumen} ≈ {float(volumen):.5f}")
        mostrar_volumen_3d(f_expr, a, b, eje=eje)

    elif opcion == '4':
        expr1 = input("\nIngresa la función exterior f(x): ")
        expr2 = input("Ingresa la función interior g(x): ")
        a = float(input("Límite inferior a: "))
        b = float(input("Límite superior b: "))
        eje = input("👉 ¿Sobre qué eje deseas visualizar el sólido 3D? (x o y): ").strip().lower()
        f_expr = parse_expr(expr1)
        ...
        print(f"✅ Volumen por revolución entre f(x) y g(x): {volumen} ≈ {float(volumen):.5f}")
